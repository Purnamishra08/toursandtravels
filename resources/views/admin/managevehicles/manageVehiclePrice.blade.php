                                                ...
                                                <div class="panel-body">
                                                    <div class="table-responsive">
                                                        <table id="example" class="table table-bordered table-striped table-hover">
                                                            ...
                                                            <tbody>
                                                                @forelse ($vehiclePrices as $key => $vehiclePrice)
                                                                    <tr>
                                                                    <td>{{ ($vehiclePrices->currentPage() - 1) * $vehiclePrices->perPage() + $loop->iteration }}</td>
                                                                        <td>{{ $vehiclePrice->vehicle_name }}</td>
                                                                        <td>{{ $vehiclePrice->destination }}</td>
                                                                        <td>{{ $vehiclePrice->price }}</td>
                                                                        <td>
                                                                                @if ($vehiclePrice->status == 1)
                                                                                    <span class="status" data-id="status-{{ $vehiclePrice->vehicleid }}">
                                                                        ...
                                                                        <td>
                                                                            <a href="{{ route('admin.managevehicles.editvehiclePrice', ['id' => $vehiclePrice->vehicleid]) }}" class="btn btn-success btn-sm" title="Edit">
                                                                                <i class="fa fa-pencil"></i>
                                                                            </a>
                                                                            @if(session('user')->admin_type == 1)
                                                                                <form action="{{ route('admin.managevehicles.deletevehiclePrice', ['id' => $vehiclePrice->vehicleid]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this vehicle?')">
                                                                                    @csrf
                                                        ...
                                                        <div class="pagination-wrapper d-flex justify-content-between align-items-center">
                                                            <p class="mb-0">
                                                                Showing {{ $vehiclePrices->firstItem() }} to {{ $vehiclePrices->lastItem() }} of {{ $vehiclePrices->total() }} entries
                                                            </p>

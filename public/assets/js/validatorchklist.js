...
function minLength(controlId, ctrlLen, fieldName)
{
    if ($('#' + controlId).val().length < ctrlLen && $('#' + controlId).val().length > 0)
    {
        viewAlert(fieldName + ' should not be less than ' + ctrlLen + ' characters !!!',controlId);
        $('#' + controlId).focus();
        return false;
    }
    return true;
...
function validateNumber(controlId)
{
    ...
    if (txtVal != '')
    {
        ...
        {
            viewAlertJs("Enter only numeric values", controlId);
            $('#' + controlId).focus();
            return false;
        }
    }
...
function validAadhar(controlId,msg) {
    var elmVal = $('#' + controlId).val();
    if (elmVal != '')
      {
...
function validPassword(controlId, txtLbl) {
    ...
    if (password != '')
    {
        if (pattern.test(password) == true)
            ...
            viewAlert(txtLbl + " Password should be alphanumeric and consist of at least one upper case letter, special character and should be 8-15 characters long", controlId);
            $('#' + controlId).focus();
            return false;
        }
    } else
...
function isOnlyCharDot(evt){
 ...
 function price_in_words(price1,label) {
...
function checkAadharLength(id, msg) {
    let inputValue = $("#" + id).val(); 
    let cnt = inputValue.length+1;  
    if (cnt >= 12) {
        setTimeout(function() {
          let x =  validAadhar(id, msg);
          console.log(x);
          if(x){
            console.log('1',$("#" + id).val());
            checkDuplicateMobileGoatAI(0,$("#" + id).val());
          }
        }, 1000);  
    }
}
function checkNumberLength(id, msg) {
    let inputValue = $("#" + id).val(); 
    let cnt = inputValue.length+1;  
    if (cnt >= 10) {
        setTimeout(function() {
          let x =  validMobileNo(id, msg);
          console.log(x);
          if(x){
            console.log('1',$("#" + id).val());
            checkDuplicateMobileGoatAI($("#" + id).val(),0);
          }
        }, 1000);  
    }
}
